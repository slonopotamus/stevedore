...
If you want to change install location from `%ProgramFiles%\Stevedore`, use `/INSTALLDIR=c:\custom\path\`.

If you only want to install specific Stevedore features, use `/ADDLOCAL=Feature1,Feature2,Feature3`.

When installing Stevedore via Chocolatey, use this syntax: `choco install stevedore --install-arguments="/ADDLOCAL=Feature1,Feature2,Feature3 /INSTALLDIR=c:\custom\path\ /quiet"`
